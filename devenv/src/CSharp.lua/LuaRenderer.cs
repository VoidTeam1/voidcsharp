...
namespace CSharpLua {
  public sealed class LuaRenderer {
    ...
    internal void Finish(Encoding encoding, string path) {
      string strToWrite = writer_.ToString();

      string fileContents = File.Exists(path) ? File.ReadAllText(path) : null;

      if (fileContents != strToWrite) {
        File.WriteAllText(path, strToWrite);
      }
    }
